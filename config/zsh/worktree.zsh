...
#   worktree new <branch> [base]  — create worktree (from base, default current branch), copy .env files, install deps
...
function worktree() {
	...
	new)
		if [ -z "$2" ]; then
			echo "Usage: worktree new <branch-name> [base]"
			echo "  base defaults to the current branch; pass e.g. 'main' or 'origin/main'"
			return 1
		fi

		local branch="$2"
		local base="${3:-HEAD}"   # default: current branch's HEAD
		local base_dir
		...
		cd "$worktree_dir"
		worktree install
		echo "Worktree ready at $worktree_dir (branched from $base)"
		;;
	install)
		local base_dir
